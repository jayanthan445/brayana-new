...
function buildDetailsTable(list,count){
    ...
    for(var i=0;i<count;i++){
        ...
        var markup = "<tr><td>"+(i+1)+"</td><td>"+list[i].name+"</td><td>"+list[i].inst_month+"</td><td>"+list[i].inst_amount+"</td><td> <a href='./button' class='btn btn-outline-danger deleteTransactionLand'  data-toggle='modal' data-target='#myModal'  data-id='"+list[i].inst_id+"' ><i class='fa fa-trash-o' aria-hidden='true'></i></a></td></tr>";
        $("table tbody").append(markup);
    }
